...
def wait_for_input_Y_or_N():
    answer = input(
        f"{BLUE}Type {RED}[Y]{BLUE} if philosopher dies type {RED}[N]{BLUE} if not: {RESET}"
    )
    while answer != "y" and answer != "n":
        answer = input(
            f"{RED}Type {RED}[Y]{BLUE} if philosopher dies type {RED}[N]{BLUE} if not: {RESET}"
        )
    return answer
...
def check_norm():
    ...
    r = subprocess.run(
        "ls", shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True
    )
    arr = r.stdout.strip().split("\n")
    for i, file_name in enumerate(arr, start=1):
        r1 = subprocess.run(
            f"norminette {file_name}",
            shell=True,
...
def check_if_not_dies():
    ...
    cmd = [
        ...
        "./philo 3 800 200 200 10",
        "./philo 4 310 150 150 10",
        "./philo 200 400 60 60 10",
        "./philo 73 400 60 60 10",
    ]
    for c in cmd:
        usleep(50000)
...
def check_leaks_on_wrong_arguments():
    ...
    cmd = [
        ...
        "./philo 4 310 -1 100",
        "./philo 4 310 100 -1",
        "./philo 4 310 100 100 -1",
        "./philo 0 310 100 100",
        "./philo 4 0 100 100",
        "./philo 4 310 0 100",
        "./philo 4 310 100 0",
        "./philo 4 310 100 100 0",
        './philo "" 310 100 100',
        './philo 4 "" 100 100',
        './philo 4 310 "" 100',
        './philo 4 310 100 ""',
        './philo 4 310 100 100 ""',
        "./philo 4 310 100 100 100 100",
        "./philo 4 310 100",
        "./philo 4 310",
        "./philo 4",
        "./philo 4 310 100 100 100 100 100",
    ...
    for c in cmd:
        ...
        if exit_code != 1:
            print(
                f"{RED}{i}. [KO]{RESET} {BLUE}EXIT [1] {RED}RETURN [{exit_code}]{RESET}"
            )
            print(f"{RED}Command: {c}{RESET}")
            answer = False
        else:
            print(
                f"{GREEN}{i}.[OK]{RESET} {BLUE}EXIT [1] {GREEN}RETURN [{exit_code}]{RESET}"
            )
        i += 1
    return answer
...
def check_helgraind_when_dies():
    ...
    cmd = [
        ...
        "./philo 4 400 200 200 10",
        "./philo 5 400 200 200 10",
        "./philo 4 310 160 150 10",
...
def chec_helgrind_when_10_times_eat():
    ...
    print(
        f"{RED}!!! IT MAY TAKE A LONG TIME BECAUSE THE TESTS ARE VERY HEAVY !!!{RESET}"
    )
    cmd = [
        "./philo 4 310 150 150 10",
        "./philo 1 800 200 200 10",
        ...
        "./philo 5 400 200 200 10",
        "./philo 4 310 150 150 10",
        "./philo 150 800 60 60 10",
        "./philo 50 800 100 100 10",
        "./philo 25 800 200 200 10",
        "./philo 10 800 200 200 10",
        "./philo 7 800 200 200 10",
        "./philo 3 800 200 200 10",
        "./philo 150 200 60 60 10",
        "./philo 50 200 60 60 10",
        "./philo 7 800 60 60 25",
        "./philo 3 800 60 60 25",
        "./philo 4 200 60 60 25",
        "./philo 25 200 60 60 25",
    ]
    helgraind_ok = "ERROR SUMMARY: 0 errors from 0 contexts"
    helgraind = "valgrind --tool=helgrind"
    for c in cmd:
